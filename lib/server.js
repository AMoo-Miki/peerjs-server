...
app._initializeWSS = function (server) {
  ...
  if (server && !(server instanceof http.Server || server instanceof https.Server)) {
    throw new Error("The server can only be an instance of http(s).Server");
  }
  ...
  var wssParam = server ? {server: server, path: path} : {noServer: true};
  this._wss = new WebSocketServer(wssParam);
